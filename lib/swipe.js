  ...
  class SwipeIndicator extends HTMLElement {
    constructor() {
      ...
      shadow.addEventListener('slotchange', () => {
        const [swipe] = slot.assignedElements()
 ...
 * Swipe Arrow
 */
const arrowTmpl = document.createElement('template')
arrowTmpl.innerHTML = `
  <style>
    :host {
      box-sizing: border-box;
...
customElements.define(
  ...
  class SwipeArrow extends HTMLElement {
    constructor() {
      super()
      const shadow = this.attachShadow({ mode: 'open' })
      shadow.appendChild(arrowTmpl.content.cloneNode(true))
...
window.addEventListener('DOMContentLoaded', () => {
  const swipes = document.querySelectorAll('.item-to-be-appended')
 ...
 */
{
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    const section = document.querySelector('#SwipeClip1')
    let i = 0
    for (let { clipIn, clipOut, path } of clips) {
      const swipe = document.createElement('swipe-clip')
